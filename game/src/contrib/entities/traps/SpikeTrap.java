...
public class SpikeTrap extends Trap{
    ...
    private void setupCollideComponent() {
        new CollideComponent(this, CollideComponent.DEFAULT_OFFSET, new Point(1, 1),
            (a, b, from) -> {
                if (b.getComponent(HealthComponent.class).isPresent()) {
                    if(!lever.getPressed()){
                        if(!activated && !entered){
                            b.getComponent(HealthComponent.class).ifPresent(
                                hc -> ((HealthComponent) hc).setCurrentHealthpoints(((HealthComponent) hc).getCurrentHealthpoints() - 2));
                        } else{
                            b.getComponent(HealthComponent.class).ifPresent(
                                hc -> ((HealthComponent) hc).setCurrentHealthpoints(((HealthComponent) hc).getCurrentHealthpoints() - 1));
                        }
                        entered = true;
                        activated = true;
