...
public class HealthPotion extends ItemData implements IOnUse {
    ...
    public void onUse(Entity e, ItemData item) {
        HealthComponent healthComponent = (HealthComponent) e.getComponent(HealthComponent.class).orElseThrow();
        InventoryComponent inventoryComponent = (InventoryComponent) e.getComponent(InventoryComponent.class).orElseThrow();
        // adds the healingAmount to currentHealth if the sum is less than maxHealth else currentHealth gets set to maxHealth
        healthComponent.setCurrentHealthpoints(Math.min(healthComponent.getCurrentHealthpoints() + healingAmount, healthComponent.getMaximalHealthpoints()));
